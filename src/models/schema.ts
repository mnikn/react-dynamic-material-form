export function validateValue(totalObjValue: any, value: any, schema: SchemaField, schemaConfig: any): any {
  ...
  if (schema.type === SchemaFieldType.Array) {
    if (Array.isArray(value)) {
      return value.map((item) => {
        return validateValue(item, item, (schema as SchemaFieldArray).fieldSchema, schemaConfig);
      });
    } else {
      return schema.config.defaultValue;
    }
      ...
      const r1 = Object.keys(value).reduce((res2: any, key) => {
        if (objFields.includes(key)) {
          res2[key] = validateValue(
            value,
            value[key],
            (schema as SchemaFieldObject).fields.find((f) => f.id === key)?.data as SchemaField,
            schemaConfig,
          );
        }
        return res2;
  ...
  if (schema.type === SchemaFieldType.String) {
    if (schema.config.needI18n) {
      if (typeof value === 'object' && value !== null) {
        ...
        return schemaConfig.i18n.reduce((res: any, item: any) => {
